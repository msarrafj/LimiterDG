...
i= mesh.coordinates.dat.data[:,:]
new_dL = (Length - Length/edge_nx)/(nx-1)
for j in range(len(i)):
    mesh_y = mesh.coordinates.dat.data[:,0][j]
    if mesh_y == Length/nx:
        mesh.coordinates.dat.data[:,0][j]= Length/edge_nx
    elif mesh_y == 0:
        print(zero)
    else:
        kk = (mesh.coordinates.dat.data[:,0][j])/(Length/nx)
        print(kk)
        mesh.coordinates.dat.data[:,0][j] = Length/edge_nx + (kk-1)* new_dL
...
file_p = File("./Output/patch_test/non_uniform_T3/NIPG+SL+FL/pres.pvd")
file_s = File("./Output/patch_test/non_uniform_T3/NIPG+SL+FL/sat.pvd")
...
for nIter in range(num_steps):
    ...
    p0.rename("Pressure","Pressure")
    s0.rename("Saturation+FL+SL","Saturation")
    # sAvg.rename("SaturationAvg+FL+SL","SatAvg")
    if counter % save_itr == 0:
        file_p.write(p0)
        file_s.write(s0)
        # file_sAvg_FL_SL.write(sAvg)
        # print ('.vtu files saved!')

    # print('Step \t solve \t FL(no_fluxCost)\t FL(all) \t SL \t convergedIter\n ')
    # print('%d \t %f \t %f \t %f \t %f \t %d '\
    #     %((counter-1), (solve_end-solve_start),(FL_end-FL_solve_start),(FL_end-FL_start),(SL_end-SL_start),convergedIter) )
        if counter == 300*save_itr:
            s0_val= s0.vector().array()
            row , col = np.shape(cell_map)
            col = 2
            s0_cell = np.zeros((row,col))
            for irow in range(row):
                for icol in range(col):
                    s0_cell[irow,icol] = s0_val[cell_map[irow,icol]]
            # print(D0_cell)
            ...
            for j in range(nx):
                for i in range(2):
                    ...
                    DG_val[2*j+i,1] = s0_cell[j,i]
            # print(DG_val)
            np.savetxt('profile/sat_300_NIPG.dat', DG_val, fmt=['%d','%1.5e'])
