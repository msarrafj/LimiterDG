...
class SignFlip:
    ...
    def HSIGN(self):
        ...
        uu, cc = np.unique(cell_map, return_counts=True)
        unique = uu[cc< 2]
        crack_facet = list(set(unique.flat) \
                    - set(left_facet.flat)\
                    - set(right_facet.flat)\
                    - set(bottom_facet.flat)\
                    - set(top_facet.flat))
        # print(crack_facet)
        ...
        # IdxHoriz = np.concatenate(np.where(normals_val_x == 0.) )
        IdxHoriz = np.concatenate(np.where((normals_val_x >-0.5)&(normals_val_x<0.5) )) # correct one

        row , col = np.shape(cell_map)
        NPLUS = np.zeros((row,col))
        for irow in range(row):
            for icol in range(col):
                if cell_map[irow,icol] in IdxHoriz:
        ...
        for irow in range(row):
            for icol in range(col):
                ...
                if facet in np.concatenate((left_facet,bottom_facet,crack_facet)): #if facet belongs to bottom/left boundary
                ...
                # elif facet in np.concatenate((top_facet,right_facet,left_crack_facet)): #if facet belongs to top/right boundary
                    ...
                    if facet in IdxHoriz: # first we address horizontal facets
                    ...
                    else: # Now we address all inner facets except horizontal
