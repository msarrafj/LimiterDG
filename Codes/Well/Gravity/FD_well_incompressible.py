...
T = 86400.*11 # final time(second) 2.5 day
...
q_I = interpolate(conditional(And(And(x[0]< 120,x[0]>40),And(x[1]<120,x[1]>40)) , 0.28e-4/3. , 0.0 ), kSpace)

q_P = interpolate(conditional(And(And(x[0]< 960,x[0]>880),And(x[1]<960,x[1]>880)) , 0.28e-4/3., 0.0 ), kSpace)
...
phi_0 = Constant(0.2)
rhow_0 = Constant(1000) #tested and correct one
rhol_0 = Constant(600)

# Gravity
g = Constant((0,-9.8)) #working use negative
...
file_p = File("./Output/Delta400pres+FL+SL.pvd")
file_s = File("./Output/Delta400sat+FL+SL.pvd")
file_v = File("./Output/Delta400vel+FL+SL.pvd")
file_error = File("./Output/Delta400err+FL+SL.pvd")
...
a_p2 = (1./dt)*(phi_0 *  (1-s) * z) * dx +\
     lmbda_l(s) * inner((K * (grad(p)+grad(Pc(s,s0)) )- K * rhol_0*g),grad(z)) * dx -\
    conditional( gt(inner(avg((K * grad(p0)- K * rhol_0*g)),n('+')),0) ,\
        inner(lmbda_l(s)('+') * avg((K * (grad(p0) + grad(Pc(s,s0)))- K * rhol_0*g)) , jump(z,n)),\
        inner(lmbda_l(s)('-') * avg((K * (grad(p0) + grad(Pc(s,s0)))- K * rhol_0*g)) , jump(z,n))\
                       ) * dS +\
    sigma/h_avg * jump(p) * jump(z)  * dS
