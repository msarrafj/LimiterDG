...
T = 20*24*3600            # final time
num_steps = 350    # number of time steps
...
q_I = interpolate(conditional(And(And(x[0]< 10,x[0]>2.5),And(x[1]<10,x[1]>2.5)) , 0.0000125, 0.0 ), kSpace)

q_P = interpolate(conditional(And(And(x[0]< 97.5,x[0]>90),And(x[1]<97.5,x[1]>90)) , 0.0000125, 0.0 ), kSpace)
...
file_p = File("./Output/pres+SL.pvd")
file_s = File("./Output/sat+SL.pvd")
file_v = File("./Output/vel+SL.pvd")
file_error = File("./Output/err+SL.pvd")
...
error.rename("Local mass balance error+SL","error")
file_error.write(error)

s0.rename("Saturation+SL","Saturation")
file_s.write(s0)

p0.rename("Pressure+SL","Pressure")
file_p.write(p0)

v0.rename("Velocity+SL","Velocity")
file_v.write(v0)
# non-linear problem
u = Function(wSpace)
u.assign(u0)
...
for nIter in range(num_steps):
    ...
    print("min:",s0.vector().array().min())
    print("max:",s0.vector().array().max())
